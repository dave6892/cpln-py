...
def list_containers_basic(client: cpln.CPLNClient, gvc_name: str) -> List[Container]:
    """
    Basic container listing example using workload-centric approach.
    ...
    """
    print(f"\n=== Listing containers in GVC: {gvc_name} (Workload-Centric) ===")
    ...
    try:
        ...
        print(f"Found {len(workloads)} workload(s), checking for containers...")

        for workload in workloads:
            try:
                # Get containers for each workload using the new method
                    ...
                    for container in containers:
                        ...
                        if container.environment_variables:
                            ...
                            for name, value in list(
                                container.environment_variables.items()
                            )[:3]:
                                print(f"  │    {name}={value}")
                        ...
                        if container.resource_limits:
                            ...
                            if container.resource_limits.get("memory"):
                                print(
                                    f"  │    Memory: {container.resource_limits['memory']}"
                                )
        ...
        if not all_containers:
            ...
            print(f"\n📊 Total containers found: {len(all_containers)}")
...
def list_containers_filtered(
    ...
    """
    Container listing with location filter using workload-centric approach.
    ...
    try:
        ...
        for workload in workloads:
            try:
                # Get containers for each workload with location filter
